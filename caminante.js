class Caminante {
  constructor(x_ = 100, y_ = 100) {
    ...
    this.y = y_;
    this.t = 20;
    this.vel = 1; // Velocidad reducida para un movimiento más lento
    this.dir = 0;
  }
  ...
  repulsion(caminantes) {
    for (let other of caminantes) {
      if (other !== this) {
        ...
        let minDistancia = this.t * 2; // Distancia mínima ajustada
        if (distancia < minDistancia) {
          ...
          this.x = constrain(this.x, 0 + 60, width - 60);
          this.y = constrain(this.y, 0 + 60, height - 60);
        }
      }
    }
  }
